          ...
          <li key={entry.id}>
            <a
              ...
              className={cn(
                ...
                activeId === entry.id && "border-primary bg-primary text-primary-foreground",
              )}
            >
...
export function Sidebar({ activeId, foundations, components, query, onQueryChange, onNavigate }: SidebarProps) {
  return (
    <aside className="border-b bg-card text-card-foreground lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r">
      <div className="flex h-full flex-col p-5 lg:p-6">
        <a href="#introduction" onClick={() => onNavigate("introduction")} className="flex items-center gap-3">
          <span className="grid size-10 place-items-center rounded-md border bg-primary font-mono text-xs font-bold text-primary-foreground">DL</span>
        ...
        <nav aria-label="Guide navigation" className="mt-6 lg:mt-10">
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto pb-1 lg:hidden" aria-label="Component catalogue">
            {[...foundations, ...components].map((entry) => <a key={entry.id} href={`#${entry.id}`} onClick={() => onNavigate(entry.id)} className={cn("shrink-0 rounded-md border bg-background px-3 py-2 text-xs font-medium text-foreground", activeId === entry.id && "border-primary bg-primary text-primary-foreground")}>{entry.name}</a>)}
          </div>
        </nav>
