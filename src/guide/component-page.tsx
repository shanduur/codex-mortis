...
function ComponentExample({ id }: { id: string }) {
  switch (id) {
    ...
    case "card":
      return <div className="grid gap-6 sm:grid-cols-2"><Card className="border-t-4 border-t-status-green"><CardHeader><Server className="size-5 text-status-green" /><CardTitle className="mt-8">System status</CardTitle><CardDescription>Rack 04 / eastern zone</CardDescription></CardHeader><CardContent><p className="font-mono text-xs font-medium uppercase tracking-wider"><span className="mr-2 inline-block size-2 rounded-full bg-status-green" />Operational</p></CardContent><CardFooter className="border-t pt-6 text-xs text-muted-foreground">Updated 12 seconds ago</CardFooter></Card><Card className="border-t-4 border-t-utility-blue"><CardHeader><Terminal className="size-5 text-primary" /><CardTitle className="mt-8">Compiler</CardTitle><CardDescription>Toolchain / release</CardDescription></CardHeader><CardContent><code className="font-mono text-sm">forge --version 0.4.1</code></CardContent></Card></div>
    case "alert":
  ...
  return (
    <>
      ...
      <section className="py-14 sm:py-20">
        <div className="mb-5 flex items-center justify-between"><h2 className="text-lg font-semibold">Preview</h2><span className="rounded-full border bg-card px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider text-muted-foreground"><span className="mr-2 inline-block size-1.5 rounded-full bg-status-green" />Light + dark ready</span></div>
        <div aria-label={`${entry.name} component examples`} className="catalogue-grid min-h-64 rounded-lg border p-6 shadow-sm sm:grid sm:place-items-center sm:p-12">
          <div className="w-full rounded-md border bg-background p-6 sm:p-8"><ComponentExample id={entry.id} /></div>
        </div>
      </section>
      ...
      <section className="grid gap-5 sm:grid-cols-2">
        <article className="rounded-lg border bg-card p-7 shadow-sm"><p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground"><span className="mr-2 inline-block size-2 rounded-full bg-status-green" />Do</p><p className="mt-8 text-sm leading-6">{copy.doThis}</p></article>
        <article className="rounded-lg border bg-card p-7 shadow-sm"><p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground"><span className="mr-2 inline-block size-2 rounded-full bg-alert-coral" />Avoid</p><p className="mt-8 text-sm leading-6">{copy.avoid}</p></article>
      </section>
